...
int loginServer(int sd, char* rec_mex, unsigned char* session_key1, unsigned char* session_key2)
{
    ...
    if (offset != (unsigned int) strlen(LOGIN_REQUEST)) {
        free(username);
        free(bufferSupp1);
        free(bufferSupp2);

        printf("Wrong login request length\n");
        return -1;
    }
    ...
    while (1)
    {
        ...
        else if (strcmp(funcSupp1, LOGOUT_REQUEST) == 0)
        {
            ...
            ret = logoutServer(sd, funcBuff, &nonce_cs, session_key1, session_key2);
            if (ret == -1)
            {
...
int logoutServer(int sd, char* rec_mex, int* nonce, unsigned char* session_key1, unsigned char* session_key2)
{
...
int downloadServer(int sock, char* rec_mex, int* nonce, unsigned char* session_key1, unsigned char* session_key2)
{
    int ret;
    size_t old_offset;
    size_t offset;
