...
int uploadClient(int sock, char* filename, unsigned char* session_key1, unsigned char* session_key2, unsigned int* nonce)
{
    ...
    ret = chdir("/home/marc/Documents/Group5_Applied_Cryptography/download");
    if (ret == -1) exit_with_failure("Can't open directory documents...", 1);
    f1 = fopen(filename, "r");
    if (!(f1))
    {
        printf("File %s doesn't exist...\n  ", filename);
        //operation_denied(sock, "The file doesn't exists", UPLOAD_DENIED, session_key1, session_key2, nonce);     //DO we need this?
        chdir("..");
        return 1;
    }
    stat(filename, &st);
