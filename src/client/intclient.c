...
int loginClient(int *sock, unsigned char** session_key1, unsigned char** session_key2, char* username, struct sockaddr_in srv_addr, int port, X509_STORE* ca_store) 
{
    ...
    *sock = createSocket();
    ...
    sgn_buff = (unsigned char*) malloc(SIGN_LEN*sizeof(unsigned char));
    ...
    {
        free_n(4, buffer, pubkey_byte, pk_buff, sgn_buff);
        EVP_PKEY_free(my_prvkey);
    ...
    free_n(5, buffer, pubkey_byte, cert_buff, pk_buff, sgn_buff);
    X509_free(serv_cert);
    ...
    free_n(3, exp_digsig, signature, buffer);
    if (ret == -1)
    {
        printf("Send failed.\n");
        return -1;
